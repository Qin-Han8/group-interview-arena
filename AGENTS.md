...
`P0` 与 `P1` 已完成并关闭，`P2` 尚未开始；当前目标版本仍为 `V0.1 Internal Validation`。阶段和版本是两套概念，不能混用，P1 收口不等于公开生产发布。
...
- 已批准的技术基线以 `docs/DECISIONS.md` 中的 `Accepted` ADR 为准；运行时和工具链版本策略必须遵守对应 ADR，不得由本机现有版本反向决定。
- `Deferred` 技术在触发重新评估并获得批准前不得启用，也不得为了未来可能使用而创建没有实际调用方的空 interface、adapter、factory、worker 或目录。
...
工具类别遵守 `docs/DECISIONS.md` 中已批准的技术基线，具体命令由实际项目配置确定并按当前任务阶段安装。不得为了体系完整提前安装未来阶段工具，不得虚构尚不存在的命令；无法执行的检查必须在交付报告中明确说明。
