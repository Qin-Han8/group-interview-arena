...
The transcript REST query filters `participant.utterance.created` by owner session and `sequence > after_sequence`，orders by the composite primary-key sequence and uses a bounded limit。Transcript sequence is a filtered view and is not contiguous。`participant.utterance.created` remains v1 and its `action_id` is projected nullable：Human preserves the original command action；AI is null。

Existing persisted P1-4 floor-event v1 facts and their action semantics remain unchanged：grant/intervention require a non-null UUID4 and release permits UUID4 or null。The additive floor-event v2 needs no schema or migration；its public serializer maps automatic scheduler/release/intervention causation to null while a directly causal Human command may retain its original action ID。Internal deterministic `SessionAction` causation remains durable/private，and F2/F3 must preserve/read historical v1 alongside v2。

Post-Human scheduling remains a separate deterministic action/decision checkpoint keyed from stable session+released-grant identity。Existing `SessionAction`、`FloorDecision` and grant/intervention children are sufficient for crash/re-entry；no orchestration/cursor table is planned。
