...
- Managed scope: P1-5R Batch R3 conversation-quality implementation within the frozen R3 production/test files plus narrow current-status synchronization；no Final Acceptance, generated/dependency/config/schema/provider/public-contract change or real provider/model call
...
- Approval state：P1-5A～P1-5F and P1-5F-1～P1-5F-4 retain historical `DONE`；P1-5F-3A/P1-5F-3B remain historical `DONE` with their recorded evidence；P1-5R is `IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1/R2-A/R2-B are `DONE`；F1/F2/visual fidelity remediation are `CLOSED`；R3 is `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING`；F3 is `OPEN / IMPLEMENTATION_COMPLETE_AWAITING_REVIEW`；Final Acceptance is `NOT_STARTED / BLOCKED_BY_R3`；R3.8 is `NOT_EXECUTED / REQUIRES_SEPARATE_EXPLICIT_USER_AUTHORIZATION`。
...
- In scope：P1-5R Batch R3 Prompt v2、bounded confirmed public discussion context、pure deterministic Persona behavior translation、production lifespan bootstrap、effective prompt selection、network-free cross-layer proof and necessary exact current-state synchronization within the frozen file map。
- Out of scope for this checkpoint：R3.8 and Final Acceptance；generated OpenAPI、schema/migration、dependency/lock、provider/model/configuration or public contract changes；real provider/model calls；later P1/P2 work。
...
- `P1-5R — Local Acceptance Remediation`：`IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1/R2-A/R2-B `DONE`；F1/F2/Visual fidelity remediation `CLOSED`；R3 `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING`；F3 `OPEN / IMPLEMENTATION_COMPLETE_AWAITING_REVIEW`；Final Acceptance `NOT_STARTED / BLOCKED_BY_R3`；R3.8 `NOT_EXECUTED / REQUIRES_SEPARATE_EXPLICIT_USER_AUTHORIZATION`；open R3 implementation findings `NONE`。
...
- Exact design contracts are frozen in [`exec-plans/P1-5R_local-acceptance-remediation.md`](exec-plans/P1-5R_local-acceptance-remediation.md)，and the executable batch/file/interface/TDD/gate plan is frozen in [`exec-plans/P1-5R_local-acceptance-remediation-implementation.md`](exec-plans/P1-5R_local-acceptance-remediation-implementation.md)。Batch R1、Batch R2-A and Batch R2-B are accepted and `DONE`；Batch R3 implementation is complete and awaits actual-source review；F3 remains OPEN awaiting that review；R3.8 was not executed；the frozen contracts remain unchanged。
