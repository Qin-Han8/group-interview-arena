...
- Managed scope: P1-5R docs-only detailed implementation-plan freeze and necessary current-state synchronization within the approved documentation set；no production/test/generated/dependency/config/schema/provider change or real provider/model call
...
- Approval state：P1-5A～P1-5F and P1-5F-1～P1-5F-4 retain historical `DONE`；P1-5F-3A/P1-5F-3B remain historical `DONE` with their recorded evidence；P1-5R is `IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1 is `NOT_STARTED / READY_FOR_IMPLEMENTATION`；R2-A/R2-B/R3 remain dependency-blocked and `NOT_STARTED`。
...
- In scope：P1-5R docs-only detailed implementation-plan freeze and necessary exact current-state synchronization within the approved documentation set。
...
- `P1-5R — Local Acceptance Remediation`：`IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1 `NOT_STARTED / READY_FOR_IMPLEMENTATION`；R2-A `NOT_STARTED / BLOCKED_BY_R1`；R2-B `NOT_STARTED / BLOCKED_BY_R2-A`；R3 `NOT_STARTED / BLOCKED_BY_R2-B`；Final acceptance `NOT_STARTED / BLOCKED_BY_R3`；F1/F2/Visual fidelity remediation/F3 remain `OPEN`；open implementation findings `NONE`。
...
- Exact design contracts are frozen in [`exec-plans/P1-5R_local-acceptance-remediation.md`](exec-plans/P1-5R_local-acceptance-remediation.md)，and the executable batch/file/interface/TDD/gate plan is frozen in [`exec-plans/P1-5R_local-acceptance-remediation-implementation.md`](exec-plans/P1-5R_local-acceptance-remediation-implementation.md)。This checkpoint implements no remediation。
