...
- Managed scope: P1-5R Batch R2-B visual-composition implementation within the frozen R2-B production/test files plus narrow current-status synchronization；no R3, backend production, generated/dependency/config/schema/provider, public-contract or real provider/model change
...
- Approval state：P1-5A～P1-5F and P1-5F-1～P1-5F-4 retain historical `DONE`；P1-5F-3A/P1-5F-3B remain historical `DONE` with their recorded evidence；P1-5R is `IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1/R2-A are `DONE`；F1/F2 are `CLOSED`；R2-B is `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING`；visual fidelity remediation is `OPEN / IMPLEMENTATION_COMPLETE_AWAITING_REVIEW`；R3 is `NOT_STARTED / BLOCKED_BY_R2-B`。
...
- In scope：P1-5R Batch R2-B truthful Interview Simulation Studio hierarchy、desktop three-column visual priority、participant/transcript/composer/task/progress fidelity、three responsive Browser viewports and necessary exact current-state synchronization within the frozen file map。
- Out of scope for this checkpoint：R3；backend production、generated OpenAPI、schema/migration、dependency/lock、config/CI/Docker、prompt/seed、provider/model or public contract changes；real provider/model calls；later P1/P2 work。
...
- `P1-5R — Local Acceptance Remediation`：`IN_PROGRESS / DESIGN_FROZEN / IMPLEMENTATION_PLAN_FROZEN`；R1/R2-A `DONE`；F1/F2 `CLOSED`；R2-B `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING`；R3 `NOT_STARTED / BLOCKED_BY_R2-B`；Final acceptance `NOT_STARTED / BLOCKED_BY_R3`；Visual fidelity remediation `OPEN / IMPLEMENTATION_COMPLETE_AWAITING_REVIEW`；F3 remains `OPEN`；open implementation findings `NONE`。
...
- Exact design contracts are frozen in [`exec-plans/P1-5R_local-acceptance-remediation.md`](exec-plans/P1-5R_local-acceptance-remediation.md)，and the executable batch/file/interface/TDD/gate plan is frozen in [`exec-plans/P1-5R_local-acceptance-remediation-implementation.md`](exec-plans/P1-5R_local-acceptance-remediation-implementation.md)。Batch R1 and Batch R2-A are accepted and `DONE`；Batch R2-B implementation is complete and awaits actual-source review；the frozen contracts remain unchanged。
