...
- Most recently completed subphase: P1-5A — `DONE` (docs-only architecture freeze)
...
- Current task gate: P1-5 — `IN_PROGRESS`; P1-5A `DONE`; implementation not started / awaiting separate explicit approval
...
阶段不是版本，P0-x/P1-x 也不是新增产品版本。P0 已完成不代表 V0.1 的全部业务能力已经实现；用户已明确批准进入 P1，P1-1～P1-4 均已完成。P1-4E final independent verdict 为 `PASS`；P1 保持 `IN_PROGRESS`。P1-5A 已完成 docs-only AI Runtime Architecture Freeze，任何 LLM/provider/runtime/utterance 实现仍未开始并等待单独明确批准。
...
`P1-5 — AI Runtime Foundation` 已启动，当前只完成首个 docs-only 子阶段：

- `P1-5A — AI Runtime Architecture Freeze`：completed；docs-only；
- 后续 implementation subphases：not started / awaiting separate explicit approval。

P1-5A 冻结 Scheduler 决定 who、AI Runtime 决定 what、provider 只负责 model I/O；冻结 provider-neutral boundary、Prompt Version + model configuration provenance、AI Participant 与 Runtime 分离、Generation Request 与 final Utterance 分离，以及 timeout/unavailable/rate-limit/partial generation 的有界幂等失败边界。Provider failure 不得改变 phase/deadline/floor/scoring 或绕过 domain service。LLM/provider/runtime、prompt engine、schema/migration、API/Realtime、memory/RAG、scoring/report、voice 和商业计费均未实现。完整边界见 [`exec-plans/P1-5_ai-runtime-foundation.md`](exec-plans/P1-5_ai-runtime-foundation.md)。
