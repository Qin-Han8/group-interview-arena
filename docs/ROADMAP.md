...
- `P1-6D — Recovery + Three-AI End-to-End Validation`: Design Freeze `DONE / DESIGN_SCOPE_FROZEN / ACTUAL_SOURCE_REVIEW_PASS`; production implementation `NOT_STARTED`; `P16D-REV-001 CLOSED`; `P16D-REV-002 CLOSED`; new findings `NONE`; open findings `NONE`; production wiring gap `NONE`; design-time STOP conditions `NONE`; production source changes `NONE`;
...
P1-6D freezes a dedicated network-free Browser Acceptance Scenario, separate from the existing layout-heavy session spec. The dedicated harness owns isolated temporary PostgreSQL, API/Web lifecycle, restart coordination, dual semantic/candidate provider behavior, persistent cross-restart signals/logs, durable verification and cleanup; the Browser spec owns only the real product journey. It must prove exactly one Human + three authoritative AI IDs, contributions from all three AI seats, legitimate phase-aware Human submissions, durable Memory revision plus candidate V3/metadata V2 provenance, three-seat Private Stance isolation, reload/reconnect, API restart, next-eligible-candidate cancellation, exact-once failure/release/continuation semantics and the accepted lifecycle through `COMPLETED`. `web:test:e2e` remains the single serial entry: the legacy harness explicitly selects the complete existing legacy Browser suite and cleans up fully before the dedicated harness explicitly selects only `p1-6d-full-session.spec.ts`; `workers = 1`, harnesses never overlap, and `gia_p16d_` allowlist/CI cleanup recognition is required. P1-6D owns the normal Memory-enabled journey and one intentional eligible-candidate cancellation only; P1-6C retains `SAFE_RAW_FALLBACK` and unsafe-context rejection as inherited focused backend regressions, rerun only if a relevant runtime/Memory seam changes. Initial review `BLOCKED` on exactly `P16D-REV-001` and `P16D-REV-002` against `group-interview-arena-review-20260831-004302.zip` / SHA-256 `7CE39C1EB4DE8314DCE7DB1CDE9809EB386E0E0CADDED8620D11B47FDB1791D0`; finding-only re-review `PASS` against `group-interview-arena-review-20260831-005931.zip` / SHA-256 `502D21B84410EDD606C806C188FFFF1FAF6C8E8ACE3F08381B86E9FA13E29A66`; `P16D-REV-001 CLOSED`; `P16D-REV-002 CLOSED`; new findings `NONE`; open findings `NONE`. Existing P1-3/P1-4/P1-5/P1-6B/P1-6C authorities and public REST/WS/Web contracts remain unchanged; production wiring gap `NONE`, production changes default to `NONE`, and design-time STOP conditions are `NONE`. P1-6D Design Freeze is `DONE / DESIGN_SCOPE_FROZEN / ACTUAL_SOURCE_REVIEW_PASS`; production implementation and P1-6E remain `NOT_STARTED`.
