...
| P0-4 数据库与迁移基础 | 使用 Docker Compose、PostgreSQL 18.x、SQLAlchemy 2.x 和 Alembic 建立数据与 migration 基础及数据库集成验证 | IN_PROGRESS |
| P0-5 最小身份边界 | 建立 V0.1 所需身份边界，不做完整公开账号产品 | TODO |
| P0-6 CI、日志与基础可观测性 | 建立自动检查、日志和基础监控能力 | TODO |
| P0-7 P0 独立验收 | 独立确认 P0 是否满足进入 P1 的条件 | TODO |

任务细节及验收条件见 [`TASKS.md`](TASKS.md)。P0-3 已通过独立最终验收并转为 `DONE`。P0-4A preflight/scope freeze、P0-4B PostgreSQL local infrastructure、P0-4C SQLAlchemy async foundation、P0-4D Alembic migration foundation 与 P0-4E PostgreSQL integration test foundation 已完成。P0-4 保持 `IN_PROGRESS`；P0-4F 等待明确批准。
