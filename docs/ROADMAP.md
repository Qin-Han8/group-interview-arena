...
- Most recently completed governance checkpoint: historical P1-5R closeout Independent Acceptance — `PASS`; reviewed committed baseline `df1cb7b67638ecf16fedcb4b105084e44fbf3b07`; exact CI run `33239991731` `SUCCESS`; governance-only closeout commit `9d6270660cb7a358a62989a6a59bab2a521fa041`
- Current task gate: P1-5 — `REOPENED / POST_ACCEPTANCE_DEFECT_REVIEW_PENDING`; P1-5R — `REOPENED / POST_ACCEPTANCE_DEFECT_REVIEW_PENDING`; P1-5R-POST-001 — `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING`; P1-5R-POST-REV-001 — `REMEDIATED / ACTUAL_SOURCE_REVIEW_PENDING`; prior P1-5R closeout and Independent Acceptance remain historical `PASS / CLOSED` evidence; R1/R2-A/R2-B/R3 remain `DONE`; F1/F2/visual fidelity remediation/F3 remain `CLOSED`; Final Composition Acceptance remains historical `PASS / DONE`; historical authorized R3.8 real-provider smoke — `EXECUTED / DEFECT_EXPOSED`; remediation real-provider calls — `0`; P1 remains `IN_PROGRESS`
...
阶段不是版本，P0-x/P1-x 也不是新增产品版本。P0 已完成不代表 V0.1 的全部业务能力已经实现；用户已明确批准进入 P1，P1 保持 `IN_PROGRESS`。P1-5A～P1-5F retain their recorded review、commit/push、CI and acceptance evidence；P1-5F-4 retains `F4-ACC-001 CLOSED` and made no real provider/model call in its accepted path。P1-5R legitimately reached `PASS / CLOSED` after R1～R3、Final Composition Acceptance and Independent Acceptance completed，and that committed closeout remains historical evidence。A subsequent explicitly authorized R3.8 real-provider local smoke exposed P1-5R-POST-001：cancellation after a generation request reached `RUNNING` could orphan the request and AI floor。The finding-only remediation is now `IMPLEMENTATION_COMPLETE / ACTUAL_SOURCE_REVIEW_PENDING` with zero additional real-provider calls；therefore P1-5R and parent P1-5 are currently `REOPENED / POST_ACCEPTANCE_DEFECT_REVIEW_PENDING`。
...
`P1-5 — AI Runtime Foundation` reached `DONE` at the committed P1-5R closeout and is currently `REOPENED / POST_ACCEPTANCE_DEFECT_REVIEW_PENDING` only for P1-5R-POST-001；the historical P1-5A～P1-5R closeout evidence remains unchanged。Its approved decomposition is：
...
- `P1-5R — Local Acceptance Remediation`：historical closeout `PASS / CLOSED`；currently `REOPENED / POST_ACCEPTANCE_DEFECT_REVIEW_PENDING` only for P1-5R-POST-001；R1/R2-A/R2-B/R3 remain `DONE`；F1/F2/visual fidelity remediation/F3 remain `CLOSED`；Final Composition Acceptance and Independent Acceptance remain historical `PASS` evidence。
...
- `P1-5R-POST-REV-001 — Real WebSocket teardown composition proof`：`REMEDIATED / ACTUAL_SOURCE_REVIEW_PENDING`；network-free Browser reload proves durable `RUNNING → FAILED / INTERNAL_ERROR → FAILED_REPLAY → INTERRUPTED` recovery，continued scheduling and no retry/utterance/release duplication。

P1-5A 冻结 Scheduler 决定 who、AI Runtime 决定 what、provider 只负责 model I/O。P1-5B～P1-5F 的 historical implementation and acceptance evidence remains unchanged，including F3A/F3B/F4 `DONE` and `F4-ACC-001 CLOSED`。P1-5R R1、R2-A、R2-B and R3 remain accepted and `DONE`，with F1、F2、visual fidelity remediation and F3 `CLOSED`；Final Composition Acceptance、Independent Acceptance and the committed P1-5R/P1-5 closeout remain historical `PASS / CLOSED` evidence。The subsequent authorized R3.8 smoke exposed only P1-5R-POST-001，whose implementation is complete and awaits actual-source review；P1-5R and parent P1-5 are therefore currently reopened only for that review gate，while P1 remains `IN_PROGRESS`。No real-provider call was made during remediation。完整边界见 [`exec-plans/P1-5_ai-runtime-foundation.md`](exec-plans/P1-5_ai-runtime-foundation.md)、[`exec-plans/P1-5R_local-acceptance-remediation.md`](exec-plans/P1-5R_local-acceptance-remediation.md) and [`exec-plans/P1-5R_local-acceptance-remediation-implementation.md`](exec-plans/P1-5R_local-acceptance-remediation-implementation.md)。
